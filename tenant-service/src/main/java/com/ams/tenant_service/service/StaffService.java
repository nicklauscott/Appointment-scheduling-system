...
public class StaffService {
    ...
    public void updateAppointmentServiceViaGrpc() {
        try {
            Optional<Staff> staff = repository.findByEmail("");
            staff.ifPresent(grpcClient::updateStaffSchedule);
        } catch (Exception e) {
    ...
    public Staff getStaff() {
        ...
        staff.setEmail("abc@io.io" + random.nextInt(100));
    ...
    public StaffResponseDTO getStaffProfile() {
        ...
        try {
            Staff staff = repository.findById(UUID.fromString(staffId))
                    .orElseThrow(() -> new StaffNotFoundException("No staff with the id: " + staffId));
            return StaffMapper.toDTO(staff);
        } catch (IllegalArgumentException e) {
            throw new StaffNotFoundException("No staff with the id: " + staffId);
    ...
    public StaffResponseDTO updateStaffProfile(StaffRequestDTO request) {
        ...
        try {
            ...
            repository.save(staff);
            grpcClient.updateStaffSchedule(staff);
            return StaffMapper.toDTO(staff);
        } catch (IllegalArgumentException e) {
            throw new StaffNotFoundException("No staff with the id: " + staffId);
        } catch (DataAccessException e) {
    ...
    public WeeklyScheduleDTO addWeeklySchedule(WeeklyScheduleDTO request) {
        ...
        try {
            ...
            return StaffMapper.toDTO(weeklyScheduleRepository.save(schedule));
        } catch (IllegalArgumentException e) {
            throw new StaffNotFoundException("No staff with the id: " + staffId);
        } catch (DataAccessException e) {
            throw new TenantNotFoundException("Invalid X-Tenant-ID value: " + TenantContext.INSTANCE.getCurrentTenant());
    ...
    public CustomScheduleDTO addCustomSchedule(CustomScheduleDTO request) {
        ...
        try {
            ...
            return StaffMapper.toDTO(customScheduleRepository.save(schedule));
        } catch (IllegalArgumentException e) {
            throw new StaffNotFoundException("No staff with the id: " + staffId);
